...
void RunTpcMonitorController(std::unique_ptr<data_monitor::DataMonitor> &daq_ptr, quill::Logger *logger, asio::io_context &io_context) {

    try {
        QUILL_LOG_DEBUG(logger, "Starting data monitor initialization...");
        daq_ptr = std::make_unique<data_monitor::DataMonitor>(io_context, kHubIp,
...
int main() {
    ...
    QUILL_LOG_INFO(logger, "Data Monitoring service starting up...");
    QUILL_LOG_INFO(logger, "Server IP: {}, Cmd Port: [{}], Status Port: [{}]", kHubIp,
                                                                        kMonitorCommandPort, kMonitorStatusPort);

    // 3. Initialize ASIO and Controller
    asio::io_context io_context;
    size_t num_io_ctx_threads = 2; // one thread for each link
    std::unique_ptr<data_monitor::DataMonitor> data_monitor_daq{};
    ...
    try {
        ...
        daq_thread = std::thread([&]() { pgrams::orchestrator::RunTpcMonitorController(data_monitor_daq, logger, io_context); });
    } catch (const std::exception& e) {
    ...
    if (pgrams::orchestrator::g_running.load()) {
        ...
        if (data_monitor_daq) { data_monitor_daq->SetRunning(false); }
    ...
    if (data_monitor_daq) { data_monitor_daq.reset(nullptr); }
